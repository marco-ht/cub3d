...
/*   Updated: 2025/12/05 05:56:55 by mpierant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_check_format_f(t_vars *v, int fd)
{
	...
	while (k < 3)
	{
		while (ft_isdigit(v->fl[i]))
			i++;
		if (i - j < 1 || i - j > 3)
			return (printf("Error: invalid floor colour format\n"),
...
static int	ft_check_format_c(t_vars *v, int fd)
{
	...
	while (k < 3)
	{
		while (ft_isdigit(v->ce[i]))
			i++;
		if (i - j < 1 || i - j > 3)
			return (printf("Error: invalid ceiling colour format\n"),
				ft_free_gnl(fd), close(fd), ft_exitclean(v), 1);
...
int	ft_check_values(t_vars *v, int fd)
{
	if (v->f[0] < 0 || v->f[0] > 255 || v->f[1] < 0 || v->f[1] > 255
		|| v->f[2] < 0 || v->f[2] > 255)
		return (printf("Error: invalid floor colour values. Range is [0,255]\n"),
			ft_free_gnl(fd), close(fd), ft_exitclean(v), 1);
	if (v->c[0] < 0 || v->c[0] > 255 || v->c[1] < 0 || v->c[1] > 255
		|| v->c[2] < 0 || v->c[2] > 255)
		return (printf("Error: invalid ceiling colour values. Range is [0,255]\n"),
			ft_free_gnl(fd), close(fd), ft_exitclean(v), 1);
	return (0);
}
